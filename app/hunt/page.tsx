...
import {
  Radar, Send, ThumbsDown, ExternalLink, TrendingUp,
  Users, DollarSign, Sparkles, RotateCcw, Building2,
} from "lucide-react";
...
function HuntScoreBadge({ score }: { score: number | null }) {
...
function HuntCard({
  ...
  return (
    <div className={`bg-white border border-slate-200 rounded-xl p-5 flex flex-col transition-all duration-200 ${
      adding ? "opacity-40 scale-[0.98] pointer-events-none" : "hover:border-slate-300 hover:shadow-sm"
    }`}>
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">
            {[result.geography, result.stage, result.founded ? `Founded ${result.founded}` : null].filter(Boolean).join(" · ")}
          </p>
        </div>
        <HuntScoreBadge score={result.huntScore} />
      </div>
      ...
      {/* Description */}
      {result.description && (
        <p className="text-xs text-slate-500 leading-relaxed mb-3 line-clamp-3">{result.description}</p>
      )}

      {/* Why it matches the hunt */}
      ...
      <div className="grid grid-cols-3 gap-1 py-2.5 border-y border-slate-100 mb-3">
        <MetricPill
          icon={DollarSign}
          label="ARR"
          value={result.arrEstimate != null ? `$${result.arrEstimate}M` : "—"}
        />
        <MetricPill
          icon={TrendingUp}
          label="Growth"
          value={result.arrGrowth != null ? `+${result.arrGrowth}%` : "—"}
        />
        <MetricPill
          icon={Users}
          label="Team"
          value={result.employees != null ? String(result.employees) : "—"}
        />
      </div>
      ...
      {result.totalFundingM != null && (
        <p className="text-[10px] text-slate-400 mb-3">
          Total raised: <span className="font-medium text-slate-600">${result.totalFundingM}M</span>
      ...
      <div className="flex gap-2 mt-auto">
        ...
        <Button
          size="sm"
          variant="outline"
          className="flex-1 text-slate-400 hover:bg-slate-50 text-xs"
          onClick={onDismiss}
        >
...
export default function HuntPage() {
  ...
  const hunt = async (q = query) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast({ title: "Hunt failed", description: data.error ?? "Unknown error", variant: "destructive" });
        return;
      }
      setResults(data.results ?? []);
      setLastQuery(data.query ?? q.trim());
      if ((data.results ?? []).length === 0) {
        toast({ title: "No results", description: "Try rephrasing your query or broadening the criteria." });
      }
    } catch (err) {
      toast({
        title: "Hunt failed",
    ...
    try {
      const res = await fetch("/api/companies", {
        ...
        body: JSON.stringify({
          ...
          stage: result.stage,
          totalFundingM: result.totalFundingM,
          employees: result.employees,
          arrEstimate: result.arrEstimate,
          arrGrowth: result.arrGrowth,
          status: "IDENTIFIED",
          priority: "MEDIUM",
          source: `Hunt: ${lastQuery}`,
        }),
      });
      if (!res.ok) throw new Error();
      setDismissed(prev => { const s = new Set(prev); s.add(key); return s; });
      toast({
        title: `${result.name} added to pipeline`,
        description: "Now visible in Companies as Identified.",
      });
    } catch {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) hunt();
  };

  const useExample = (example: string) => {
    setQuery(example);
    setTimeout(() => hunt(example), 0);
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-2.5 mb-1">
        ...
        <h1 className="text-xl font-bold text-slate-900">Hunt</h1>
      </div>
      <p className="text-sm text-slate-500 mb-6 max-w-2xl">
        Run targeted searches when you want to go deep on a specific niche, sector, or set of criteria.
        Results are ephemeral — add the ones you like directly to your pipeline.
      </p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6 shadow-sm">
        <textarea
          ...
          rows={3}
          placeholder={'Describe what you\'re looking for… e.g. "Manufacturing software companies with 25–75 employees founded 2020–2023 that serve interesting parts of the manufacturing value chain"'}
          value={query}
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100">
          ...
          >
            {loading ? (
              <>
                <Radar size={13} className="mr-1.5 animate-pulse" />
                Hunting…
              </>
            ) : (
              <>
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-white border border-dashed border-slate-200 rounded-xl p-5 animate-pulse flex flex-col gap-3">
              ...
              <div className="flex justify-between mt-1">
                {Array.from({ length: 3 }).map((_, j) => <div key={j} className="h-3 bg-slate-100 rounded w-10" />)}
              </div>
        ...
        <>
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-sm font-medium text-slate-700">
                {results.filter(r => !dismissed.has(r.name)).length} results
              </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {results.map(result => (
              <HuntCard
                ...
                onDismiss={() => setDismissed(prev => { const s = new Set(prev); s.add(result.name); return s; })}
          ...
          {results.every(r => dismissed.has(r.name)) && (
            <div className="text-center py-16 text-slate-400 text-sm">
              ...
              <button className="text-blue-600 hover:underline" onClick={() => { setResults([]); setQuery(""); }}>
                Start a new search
              </button>
            </div>
          )}
